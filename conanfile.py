...
class SdlConan(ConanFile):
    ...
    url = "https://github.com/conanos/SDL"
    ...
    def source(self):
        url_ = 'https://github.com/ShiftMediaProject/SDL/archive/release-{version}.tar.gz'
    ...
    def build(self):
        if self.settings.os == 'Windows':
            with tools.chdir(os.path.join(self._source_subfolder,"SMP")):
                ...
                msbuild.build("libsdl.sln",upgrade_project=True,platforms={'x86': 'Win32', 'x86_64': 'x64'},build_type=build_type)
    ...
    def package(self):
        if self.settings.os == 'Windows':
            ...
            shutil.copyfile(os.path.join(self.build_folder,self._source_subfolder,"sdl2.pc.in"),
                            os.path.join(self.package_folder,"lib","pkgconfig", "sdl2.pc"))
            lib = "-lsdl2d" if self.options.shared else "-lsdl2"
            replacements = {
                "@prefix@"                        : self.package_folder,
                "@exec_prefix@"                   : "${prefix}/bin",
                "@libdir@"                        : "${prefix}/lib",
                "@includedir@"                    : "${prefix}/include",
                "@SDL_VERSION@"                   : self.version,
                "@SDL_RLD_FLAGS@"                 : "",
                "@SDL_LIBS@"                      : lib,
                "@SDL_STATIC_LIBS@"               : "",
                "@SDL_CFLAGS@"                    : "",
                "-I${includedir}/SDL2"            : "-I${includedir}/SDL"
            }
            for s, r in replacements.items():
                tools.replace_in_file(os.path.join(self.package_folder,"lib","pkgconfig", "sdl2.pc"),s,r)
